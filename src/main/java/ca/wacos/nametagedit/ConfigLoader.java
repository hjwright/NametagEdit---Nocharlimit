...
class ConfigLoader {
	 ...
	 *         {@link String} key, and another {@link java.util.LinkedHashMap}
	 *         as the value, containing both the operation and the actual
	 *         configuration value.
	 */
	 ...
	 * 
	 * @param target
	 *            the target file to generate and load.
	 * @return a {@link java.util.LinkedHashMap} of configurations, with a
	 *         {@link String} key, and another {@link java.util.LinkedHashMap}
	 *         as the value, containing both the operation and the actual
	 *         configuration value.
	 ...
	 */
	static boolean parseBoolean(String name, String operation,
			LinkedHashMap<String, LinkedHashMap<String, String>> config,
			boolean d) {
		if (config.containsKey(name) && config.get(name).containsKey(operation)) {
			...
			System.out.println("[NametagEdit] Could not parse boolean for \""
					+ name + " " + operation
					+ "\" in config.txt, value given: " + value
					+ ", defaulting to " + d);
		}
		return d;
	}
