...
public class NametagAPI {
     ...
     * to prevent it from clashing with the PlayerJoinEvent in NametagEdit.
     *
     ...
     * Sets the custom given prefix and suffix to the player, overwriting any
     * existing prefix or suffix. If a given prefix or suffix is null/empty, it
     * will be removed from the player.<br><br>
     *
     ...
     * group node that applies to that player.
     * </br></br>
     * This method schedules a task with the request to change the player's name
     * to prevent it from clashing with the PlayerJoinEvent in NametagEdit.
     *
                ...
                if (targetPlayer != null) {
                    ...
                    for (String key : NametagEdit.groups.keySet().toArray(new String[NametagEdit.groups.keySet().size()])) {
                        if (targetPlayer.hasPermission(key)) {
                            ...
                            NametagCommand.setNametagHard(player, prefix, suffix, NametagChangeReason.GROUP_NODE);
                    ...
                    switch (permissions) {
                        case "gm":
                            for (String key : NametagEdit.groups.keySet().toArray(new String[NametagEdit.groups.keySet().size()])) {
                                if (NametagEdit.groupManager.getWorldsHolder().getWorldPermissionsByPlayerName(player).permission(player, key)) {
                                    String prefix = NametagEdit.groups.get(key).get("prefix");
                                    String suffix = NametagEdit.groups.get(key).get("suffix");
                                    if (prefix != null) {
                                        prefix = NametagUtils.formatColors(prefix);
                        ...
                        case "pex":
                            for (String key : NametagEdit.groups.keySet().toArray(new String[NametagEdit.groups.keySet().size()])) {
                                for (String perm : PermissionsEx.getUser(player).getPermissions(player)) {
                                    if (perm == null ? key == null : perm.equals(key)) {
                                        String prefix = NametagEdit.groups.get(key).get("prefix");
                                        String suffix = NametagEdit.groups.get(key).get("suffix");
                                        if (prefix != null) {
                                            prefix = NametagUtils.formatColors(prefix);
