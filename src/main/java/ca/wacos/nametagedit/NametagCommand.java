...
class NametagCommand implements CommandExecutor {
		...
		if (cmd.getName().equalsIgnoreCase("ne")) {
			...
			if (args.length >= 2) {
				...
				if (operation.equalsIgnoreCase("prefix")
					...
					if (targetPlayer != null)
						setNametagSoft(targetPlayer.getName(), prefix, suffix,
								reason);
					if (targetPlayer != null)
	...
	/**
	 * Triggers a plugin update if the given
	 * {@link org.bukkit.command.CommandSender} has created an update task
	 * previously by calling {@link #update(org.bukkit.command.CommandSender)}
	 * 
	 * @param sender
	 *            the {@link org.bukkit.command.CommandSender} to execute from
	 * @return true
